...
const TOOLS = [
  ...
  // ---- cross-machine migration ----
  {
    ...
    inputSchema: { type: 'object', properties: { path: { type: 'string' }, passphrase_file: { type: 'string', description: 'File whose contents are the encryption passphrase. Omit to write an UNENCRYPTED bundle.' }, no_sessions: { type: 'boolean' }, no_providers: { type: 'boolean' }, agents: { type: 'boolean', description: 'Also carry other agents\' home-level memory (markdown, no secrets).' }, agent_config: { type: 'boolean', description: 'Also carry other agents\' config files (MCP/settings), secret-scanned + redacted.' }, agent_config_secrets: { type: 'boolean', description: 'Carry the REAL agent-config keys (not redacted). Requires passphrase_file.' }, agent_ids: { type: 'array', items: { type: 'string', enum: ['cursor', 'gemini', 'codex'] }, description: 'Limit agents/agent_config to these agent ids (default: all present).' }, confirm: confirmProp.confirm }, required: ['path', 'confirm'], additionalProperties: false },
